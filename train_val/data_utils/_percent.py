...
def num_percent(percent, coco_dir):
    ...
    if os.path.exists(output_txt):
        ...
        target_annotations_count = int(total_annotations * float(percent) / 100)
        print("# target_annotations_count=", target_annotations_count)

        # 获取每个类别的标注比例
        category_ratios = {
            cat['id']: len(category_to_annotations[cat['id']]) / total_annotations for cat in categories
        }

        # 按照类别比例抽取数据
        selected_annotations = []
        for cat_id, ratio in category_ratios.items():
            # 计算当前类别需要抽取的标注数量
            target_count_for_category = int(target_annotations_count * ratio)
            selected_annotations.extend(
                random.sample(category_to_annotations[cat_id], min(target_count_for_category, len(category_to_annotations[cat_id])))
            )
