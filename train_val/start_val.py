...
yolo val model='D:\\code\\yolov10\\runs\\detect\\train\\weights\\best.pt' data=coco.yaml batch=128
''' 
...
if __name__ == '__main__':
    ...
    '''
    在 Windows 中，multiprocessing 模块要求在启动新的进程时，脚本的入口点必须通过 if __name__ == '__main__': 来保护。
    这是因为 Windows 不支持 Unix-like 的 fork 操作，而是通过重新导入模块来启动新进程。
    如果没有这个保护，模块可能会被重复导入，从而导致 RuntimeError。
    '''
    from ultralytics import YOLOv10
