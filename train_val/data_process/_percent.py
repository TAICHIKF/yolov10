...
def num_percent(percent=1):
    # 设置路径
    coco_json = r"D:\code\datasets\coco\annotations\instances_train2017.json"  # COCO标注文件路径
    output_json = fr"D:\code\datasets\coco\annotations\instances_train2017_{percent}percent.json"         # 抽取后保存的标注文件
    output_txt = fr"D:\code\datasets\coco\train2017_{percent}percent.txt"  # 输出的图像路径txt文件
...
'''
2024.12.19
抽取完成！新的标注文件保存在: D:\code\datasets\coco\annotations\instances_train2017_1percent.json
图像数量: 8022，标注数量: 8559
图像路径已保存到: D:\code\datasets\coco\train2017_1percent.txt
person: 2624 个标注
bicycle: 71 个标注
car: 438 个标注
motorcycle: 87 个标注
